...
int client(int procnum, char **proc) {
	...
	fp = popen("mpstat -w |grep ALL |awk '{print 100-$15}'", "r"); //전체 cpu 정보
	if(fp == NULL) {
		perror("error: ");
		exit(0);
	}
	...
	fp = popen("svmon|grep memory |awk '{print $4/$2*100}'", "r"); //전체 mem 정보
	if(fp == NULL) {
		perror("error: ");
		exit(0);
	}
	...
	for(i=1; i<procnum+1; i++) {
		char procstr[] = "ps -eo pcpu,pmem,comm|grep "; //프로세스별 cpu, mem 정보
